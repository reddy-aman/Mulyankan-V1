<x-app-layout>
    <div class="flex h-screen">
        ...
        <!-- Main Content -->
        <div class="flex-1 flex flex-col p-6 bg-gray-50" x-data="{ openModal: false, showSingleUser: false, showCSVUpload: false, showEditModal: false, editUserData: {} }">
            <!-- Top Bar -->
            <div class="bg-white shadow-md px-6 py-4 rounded-lg flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Roster</h1>
</div>
...
<div class="">
            <!-- Top-level container with Alpine data and listener for close-modal event -->
                ...
                <!-- Dropdown for filtering & Search bar -->
                            ...
                            <form method="GET" action="{{ route('courses.roster', ['id' => $course->id]) }}" class="flex w-full">
                                <div class="flex-1">
                                    <select name="role" class="border border-gray-300 px-4 py-2 rounded shadow-sm"
                                        ...
                                        <option value="students" {{ request('role') == 'students' ? 'selected' : '' }}>Students</option>
                                        <option value="instructors" {{ request('role') == 'instructors' ? 'selected' : '' }}>Instructors</option>
            ...
            <div class="bg-white shadow-md px-6 py-4 rounded-t-lg flex justify-end space-x-3">
                <button class="px-4 py-2 bg-white text-gray-800 border border-gray-300 rounded shadow-md hover:bg-gray-100">
                    Sync LMS Roster
                </button>
                ...
                <form action="{{ route('courses.rosterDownload', ['id' => $course_id]) }}" method="GET">
                    <button type="submit" class="px-4 py-2 bg-white text-gray-800 border border-gray-300 rounded shadow-md hover:bg-gray-100">
                        Download Roster
                    </button>
                </form>
                ...
                <button class="px-4 py-2 bg-blue-600 text-white border border-blue-600 rounded shadow-md hover:bg-blue-700"
            ...
            <div x-show="openModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50"
                ...
                <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full" @click.stop>
                    ...
                    <div class="mt-6 text-right">
                        <button class="px-4 py-2 bg-gray-200 border rounded hover:bg-gray-300"
                                @click="openModal = false">
                            Cancel
                        </button>
                    </div>
                ...
                <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full" @click.stop>
                    ...
                    <form id="csvUploadForm" class="mt-4">
                        ...
                        <p class="mt-4 text-lg font-semibold text-red-600 bg-red-100 p-3 rounded-lg border border-red-400">
                            ⚠ CSV Format: <strong>Name, Email, Student ID, Role</strong>
                            (Role must be <em>Student, Instructor, or TA</em>)
                        </p>
                        <div class="mt-6 flex justify-end space-x-2">
                            <button type="button"
                                    class="px-4 py-2 bg-gray-200 border rounded hover:bg-gray-300"
