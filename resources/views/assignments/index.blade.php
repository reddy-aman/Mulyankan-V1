                                ...
                                <tr class="hover:bg-gray-50">
                                            ...
                                            @foreach ($stages as $index => $stage)
                                                <div class="flex items-center">
                                                    ...
                                                    @php
                                                        switch ($stage) {
                                                            case 'Edit Outline':
                                                                $url = route('assignments.annotateTemplate', [
                                                                    'assignment' => $assignment->id,
                                                                ]);
                                                                break;
                                                            case 'Submission Uploaded':
                                                                $url = route('assignments.uploadForm', $assignment->id);
                                                                break;
                                                    ...
                                                    <div class="flex items-center">
                                                        <a href="{{ $url }}"
                                                            class="{{ $index === $currentStage + 1 ? 'text-blue-600 font-semibold' : 'text-gray-700 hover:text-blue-600' }}">
                                                            {{ $stage }}
                                                        </a>
                                                        {{-- Add Work in Progress label for Submission Graded and Grade Reviewed --}}
                                                        @if (in_array($stage, ['Submission Graded', 'Grade Reviewed']))
                                                            <span
                                    ...
                                    <td class="px-4 py-2 border-b text-right">
                                        <div class="relative inline-block">
                                            ...
                                            <!-- Dropdown -->
                                            <div
                                                class="action-menu hidden absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded shadow-md z-50 max-h-96 overflow-y-auto">
                                                <a href="{{ route('assignments.edit', $assignment->id) }}"
                                                    class="block px-4 py-2 whitespace-nowrap text-gray-700 hover:bg-gray-100">
                                                    Edit Assignment
                                                </a>
                                                <form method="POST"
                                                    action="{{ route('assignments.deleteAssignment', $assignment->id) }}"
